(function ($) {
		...
		if (!settings.init) {
			...
			$.each(timespan, function (k, v) {
				...
				if (settings.labels) {
					...
					label = $('<span/>').addClass('label').text((v === 1 ? $.fn.countdown.singularize(k) : $.fn.countdown.translatePortuguese(k)));
			...
			$.each(timespan, function (k, v) {
				...
				$('.label', '#' + k).text((v === 1 ? $.fn.countdown.singularize(k) : $.fn.countdown.translatePortuguese(k)));
			});
		}
	...
	$.fn.countdown.singularize = function (str) {
		...
		var label = str.substr(0, str.length - 1);
		label = $.fn.countdown.translatePortuguese(label);

		return label;
	};

	//Translate label to Portuguese
	$.fn.countdown.translatePortuguese = function (label) {
		var label = (label === 'days') ? 'dias' :
					(label === 'day') ? 'dia' :
					(label === 'hours') ? 'horas' :
					(label === 'hour') ? 'hora' :
